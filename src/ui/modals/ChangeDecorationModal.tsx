...
const UserSummaryItem = webpack.getBySource(["defaultRenderUser", "showDefaultAvatarsForNullUsers"])
...
function usePresets() {
  ...
  React.useEffect(() => { getPresets().then(setPresets) }, [])
  return presets
}
        ...
        <div className={cl("change-decoration-modal-footer-btn-container")}>
          <Button
            onClick={() => Alerts.show({
              ...
              cancelText: "Cancel",
              onConfirm() {
                useAuthorizationStore.getState().remove(users.getCurrentUser().id);
                props.onClose();
              }
            })}
            color={Button.Colors.PRIMARY}
